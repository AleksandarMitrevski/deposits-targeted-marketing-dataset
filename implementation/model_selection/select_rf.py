...
number_of_trees_candidates = [10, 50, 100, 150, 200, 300, 400, 500]
results_df = pd.DataFrame(np.zeros((2, len(number_of_trees_candidates))))
i = 0
for number_of_trees in number_of_trees_candidates:
    ...
    for j in range(len(random_split)):
        ...
        classifier = RandomForestClassifier(n_estimators=number_of_trees, max_depth=16, min_samples_split=22, min_samples_leaf=10)
    ...
    print("step {}: {} - {}".format(i + 1, number_of_trees, current_auc))
    results_df.iloc[0, i] = number_of_trees
    results_df.iloc[1, i] = current_auc
    i += 1

results_df.to_csv("select_rf.csv")
